...
```bash
make build-brs VERSION_brs=v0.8.0 DATE=2026-06-12
ls build/
# → BRS-v0.8.0-20260612.pdf
```

Every PDF this repo emits (`build-brs`, `build-brs-ts`) is named to the
ARC-compliant `<short>-v<X.Y.Z>-<YYYYMMDD>.pdf` pattern by construction —
there is no separate "ARC build" step.

### 4.3 Multi-document repos

This repo builds **two** independently-versioned specifications — the BRS
spec (`brs.adoc`, tag-driven `VERSION_brs`) and the BRS Test Specification
(`brs-ts.adoc`, static `VERSION_brs-ts`) — each with its own `SPEC_SHORT`
(`BRS`, `BRS-TS`). Only BRS is submitted to ARC, with its tag
lineage and its own filename/title-page identity.
...
This repo was migrated to the `docs-spec-template` toolchain; see `ANTORA.md`
for how the dual PDF/Antora build works in this repo specifically, and
`riscv/docs-spec-template`'s own `MIGRATION.md` for the generic checklist this
migration followed.
